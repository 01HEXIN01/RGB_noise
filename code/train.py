...
class Train:
    def __init__(self):
        ...
        # 初始化一个 DataFrame，用于保存训练结果
        self.results = pd.DataFrame(
            columns=['Iteration', 'Total Loss', 'RPN Loss Cls', 'RPN Loss Box', 'Loss Cls', 'Loss Box',
                     'Speed (s/iter)'])
    ...
    def train(self):
        ...
        while iter < cfg.FLAGS.max_iters + 1:
            ...
            if iter % (cfg.FLAGS.display) == 0:
                ...
                # 保存当前迭代的结果到 DataFrame 中
                new_row = {
                    'Iteration': iter,
                    'Total Loss': total_loss,
                    'RPN Loss Cls': rpn_loss_cls,
                    'RPN Loss Box': rpn_loss_box,
                    'Loss Cls': loss_cls,
                    'Loss Box': loss_box,
                    'Speed (s/iter)': timer.average_time
                }
        ...
        # 训练完成后，将结果保存到 CSV 文件
        output_path = r'D:\study\Python code\test3\Result'
